...
- Added experimental watch mode
  ...
  subsequent cycles after change detected (`-j`/`--jobs`). Subsequent cycles
  usually need to regenerate only a small number of tiles, so setting `--jobs`
  to a smaller value than `--jobs-initial` may be advantageous.
